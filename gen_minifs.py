...
static int fs_read_super(void) {
    unsigned char buf[MINIFS_BLOCK_SIZE];
    if (block_read(0, buf) < 0) return -1;
    kmemcpy(&fs_sb, buf, sizeof(MiniFSSuper));
    if (fs_sb.magic != MINIFS_MAGIC) return -1;
    if (fs_sb.version != MINIFS_VERSION) return -1;
    if (fs_sb.block_size != MINIFS_BLOCK_SIZE) return -1;
    return 0;
}
...
/* ---- Journal (stub for now) ---- */

void minifs_journal_begin(unsigned int txn_id) { fs_next_txn = txn_id; }
void minifs_journal_add_block(unsigned int block) { (void)block; }
int minifs_journal_commit(unsigned int txn_id) { (void)txn_id; return 0; }
...
int minifs_dir_lookup(int dir_ino, const char *name) {
    ...
    for (b = 0; b < total_blocks; b++) {
        ...
        while (off < MINIFS_BLOCK_SIZE) {
            ...
            if (de->inode != 0 && fs_namecmp(((char *)de + sizeof(MiniFSDirEntry)), ((char *)de + sizeof(MiniFSDirEntry))_len, name) == 0)
                return (int)de->inode;
            off += de->rec_len;
        }
    }
...
int minifs_dir_add_entry(int dir_ino, const char *name, int child_ino,
    ...
    unsigned int entry_len = roundup4(sizeof(MiniFSDirEntry) + namelen);
    unsigned int total_blocks, b;
    if (fs_read_inode((unsigned int)dir_ino, &dir) < 0) return -1;
    if (!(dir.mode & MINIFS_S_IFDIR)) return -1;
    ...
    for (b = 0; b < total_blocks; b++) {
        ...
        while (off < MINIFS_BLOCK_SIZE) {
            ...
            if (de->inode == 0 && de->rec_len >= entry_len) {
                ...
                ((char *)de + sizeof(MiniFSDirEntry))_len = (unsigned char)namelen;
                de->file_type = type;
                kmemcpy(((char *)de + sizeof(MiniFSDirEntry)), name, namelen);
    ...
    if (last_de) {
        unsigned int avail = MINIFS_BLOCK_SIZE -
            ((unsigned long)last_de - (unsigned long)buf) - last_((char *)de + sizeof(MiniFSDirEntry))_len
            - sizeof(MiniFSDirEntry);
        avail = roundup4(avail);
        if (last_de->inode != 0) {
            unsigned int tail = roundup4(sizeof(MiniFSDirEntry) + last_((char *)de + sizeof(MiniFSDirEntry))_len);
            avail += tail - last_de->rec_len;
            MiniFSDirEntry *removed_tail_de = (MiniFSDirEntry *)((char *)last_de + tail);
            if (avail >= entry_len) {
                ...
                ((char *)ne + sizeof(MiniFSDirEntry))_len = (unsigned char)namelen;
                ne->file_type = type;
                ne->rec_len = (unsigned short)(saved + last_((char *)de + sizeof(MiniFSDirEntry))_len +
                    sizeof(MiniFSDirEntry) - tail);
                kmemcpy(((char *)ne + sizeof(MiniFSDirEntry)), name, namelen);
                return block_write(last_phys, buf);
            }
        } else {
            if (avail >= entry_len) {
                last_de->inode = (unsigned int)child_ino;
                last_((char *)de + sizeof(MiniFSDirEntry))_len = (unsigned char)namelen;
                last_de->file_type = type;
                kmemcpy(last_((char *)de + sizeof(MiniFSDirEntry)), name, namelen);
                return block_write(last_phys, buf);
            }
        }
    }
    ...
    n((char *)de + sizeof(MiniFSDirEntry))_len = (unsigned char)namelen;
    nde->file_type = type;
    nde->rec_len = (unsigned short)MINIFS_BLOCK_SIZE;
    kmemcpy(n((char *)de + sizeof(MiniFSDirEntry)), name, namelen);
...
int minifs_dir_read(int dir_ino, int index, MiniFSDirEntry *out) {
    MiniFSInode dir;
    unsigned int total_blocks, b;
    int count = 0;
    if (fs_read_inode((unsigned int)dir_ino, &dir) < 0) return -1;
        ...
        while (off < MINIFS_BLOCK_SIZE) {
            ...
            if (de->inode != 0) {
                if (count == index) {
                    ...
                    kmemcpy(out->name, ((char *)de + sizeof(MiniFSDirEntry)), ((char *)de + sizeof(MiniFSDirEntry))_len);
                    out->name[((char *)de + sizeof(MiniFSDirEntry))_len] = 0;
                    return 0;
                }
                count++;
            }
...
int minifs_mount(void) {
    ...
    unsigned int kernel_sectors;
    unsigned int ibm_blocks, bbm_blocks, it_blocks;
    unsigned int i;
    ...
    {   unsigned int ksize = (unsigned int)(&ramdisk_end[0] - &ramdisk_start[0]);
        if (ksize == 0) ksize = 1;
        kernel_sectors += (ksize + IDE_SECTOR_SIZE - 1) / IDE_SECTOR_SIZE;
    }

    fs_lba_start = kernel_sectors;
    unsigned int fs_sectors = total_sectors - fs_lba_start;
    if (fs_sectors < SECTORS_PER_BLOCK * 4) {
        kprintf("minifs: not enough space after kernel (%u sectors)\n", fs_sectors);
        return -1;
    }
