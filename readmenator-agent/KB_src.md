...
- Symbols:
  ...
  - `AES_MAGIC0` (macro, line 38) `#define AES_MAGIC0`
  ...
  - `AES_BLOCK` (macro, line 44) `#define AES_BLOCK`
...
- Symbols:
  ...
  - `CP_BUF_SIZE` (macro, line 7) `#define CP_BUF_SIZE`
...
- Symbols:
  ...
  - `gettid` (function, line 21) `* * The same run smokes gettid (Phase 0.4: the two workers must observe * distinct tids, never the constant 1) and getrandom (Phase 0.5: it * must fill the buffer and report the count, never EFAULT). `
  - `FP_ITERS` (macro, line 38) `#define FP_ITERS`
...
- Symbols:
  ...
  - `FREEDOM_HOPS_MAX` (macro, line 74) `#define FREEDOM_HOPS_MAX`
...
- Symbols:
  ...
  - `JS_MAX_NODES` (macro, line 27) `#define JS_MAX_NODES`
  - `JS_POOL` (macro, line 28) `#define JS_POOL`
  - `JS_NULL` (macro, line 30) `#define JS_NULL`
  ...
  - `JS_SEEK_END` (macro, line 37) `#define JS_SEEK_END`
...
- Symbols:
  ...
  - `kernel` (function, line 6) `* * The codec lives in the kernel (lz4_kernel.c, the same one MiniFS uses), so * these tools are thin front-ends over the two MiniOS syscalls 216 * (lz4_compress) and 217 (lz4_decompress), which mirro`
...
- Symbols:
  ...
  - `threads` (function, line 3) `* * Ten threads (1 main + 5 producers + 4 consumers) share one address * space through thread_spawn (MiniOS syscall 225): a bounded buffer, * a spin mutex and Mesa-style spin-waits. Workers never touc`
