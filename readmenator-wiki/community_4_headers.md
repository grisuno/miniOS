...
This community groups 3 file(s) rooted at `kernel` with dominant language c (cohesion 0.40). Central symbols: `ABI_BAD_FORMAT`, `ABI_CHECKSUM_MISMATCH`, `ABI_H`, `ABI_MANIFEST_MAX`, `ABI_MANIFEST_NAME`, `ABI_NO_MANIFEST`, `ABI_OK`, `ABI_VERSION_MISMATCH`. Core file: `headers/abi.h` (10 symbols). Documented purpose: Docstring: kernel/abi.c -- Boot-time ABI manifest gate..
...
| `tests/test_abi.c` | c | testing | 3 | yes |
...
- `abi_verify` (function, `headers/abi.h:24`) `int abi_verify(const char *text, long version, unsigned long checksum);` - define ABI_MANIFEST_NAME "etc/abi" define ABI_MANIFEST_MAX 64
